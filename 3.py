...
class Cell:
    ...
    def make_order(self, cells=5):
        cell = ''
        if self.number_cells > 0:
            for c in range(self.number_cells // cells):
                cell += '*' * cells + r'\n'
            if self.number_cells % cells == 0:
                cell = cell.rstrip(r'\n')
            else:
                cell += '*' * (self.number_cells % cells)
        else:
            cell = 'Недопустимое значение ячеек'
        return cell


cell_1 = Cell(25)
print(f'Клетка_1 = {cell_1.number_cells} яч. => {cell_1.make_order()}')

cell_2 = Cell(10)
print(f'Клетка_2 = {cell_2.number_cells} яч. => {cell_2.make_order()}')

print(f'Клетка_1 + Клетка_2 = {cell_1.number_cells + cell_2.number_cells} яч. => {(cell_1 + cell_2).make_order()}')
print(f'Клетка_1 - Клетка_2 = {cell_1.number_cells - cell_2.number_cells} яч. => {(cell_1 - cell_2).make_order()}')
print(f'Клетка_1 * Клетка_2 = {cell_1.number_cells * cell_2.number_cells} яч. => {(cell_1 * cell_2).make_order()}')
print(f'Клетка_1 / Клетка_2 = {cell_1.number_cells // cell_2.number_cells} яч. => {(cell_1 / cell_2).make_order()}')
