...
while True:
    ...
    unit = []
    for k in product:
        if k[1].get('название') not in name:
            name.append(k[1].get('название'))
        if k[1].get('цена') not in price:
            price.append(k[1].get('цена'))
        if k[1].get('количество') not in num:
            num.append(k[1].get('количество'))
        if k[1].get('ед') not in unit:
            unit.append(k[1].get('ед'))
        print(k)

    analytics_product = {'название': name, 'цена': price, 'количество': num, 'ед': unit}
    print(analytics_product)
